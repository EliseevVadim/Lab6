...
namespace LW5
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            byte pos = 3;
            bool flag = true;
            if (!File.Exists(@"d:\файлы\6.1\lab.dat"))
            {
                FileStream file = new FileStream(@"d:\файлы\6.1\lab.dat", FileMode.Create);
                BinaryWriter writer = new BinaryWriter(file);
                writer.Write(agros[0].name); writer.Write(agros[0].sqare); writer.Write(agros[0].productivity); writer.Write(agros[0].type.ToString());
                writer.Write($"\n{agros[1].name}"); writer.Write(agros[1].sqare); writer.Write(agros[1].productivity); writer.Write(agros[1].type.ToString());
                writer.Write($"\n{agros[2].name}"); writer.Write(agros[2].sqare); writer.Write(agros[2].productivity); writer.Write(agros[2].type.ToString());
                writer.Close();
                ...
                while (flag)
                {
                    ...
                    switch (inputanswer)
                    {
                        case "1":
                            ...
                            //}
                            Console.WriteLine(File.ReadAllText(@"d:\файлы\6.1\lab.dat"));
                            CR();
                            CC();
                            break;
                        case "2":
                        ...
                        case "6":
                            ...
                            break;
                        case "7":
                            FileStream end = new FileStream(@"d:\файлы\6.1\lab.dat", FileMode.Open);
                            BinaryWriter writer1 = new BinaryWriter(end);
                            for (int i=0; i<pos; i++)
                            {
                                writer1.Write(agros[i].name);
                                writer1.Write(agros[i].type.ToString());
                                writer1.Write(agros[i].sqare);
                                writer1.Write(agros[i].productivity);
                                writer1.Write("\n");
                            }
                            writer1.Close();
                            flag = false;
